...
//get home page
 ...
 //get all article  from corresponding subject
...
//First User and passord is encrypted and also check the same email with in db..
     ...
     if(!personInfo.email ||!personInfo.password || !personInfo.name || !personInfo.address || !personInfo.phoneno)
    {
    ...
    else
     {
        Person.find(function(err, response)
        {
            ...
            {
              let hash;
                 var plaintextPassword=personInfo.password;

                 //function call fro encrpy password 
              ...
              async function hashPassword(plaintextPassword)
               {
                   hash = await bcrypt.hash(plaintextPassword, 10);
                   console.log(hash);
                   var  current_date=Date.now();
                   var newPerson = new Person
                  ({
                    email:personInfo.email,
                    password:hash,           // Store hash in the database
                    name: personInfo.name,
                    address: personInfo.address,
                    phoneno: personInfo.phoneno,
                    current_date:current_date,
...
//after login submit check the same email and password and also admin ban or not
...
router.post('/login',function(req,res)
{
    ...
    if(!req.body.email || !req.body.password)
    {
     ...
     else
     {
           ...
           //check password comparison function

         findemail();
         //function password decrpt and compare the input password and db password

         //check the corresponding email in db
         async function findemail()
         {
            const user =await Person.findOne({ $and:[{email: req.body.email},{"approved":1 }]})
            console.log(user);
            if(!user)
           {
          ...
          const login=dataslogin.filter(function(user)
           {
              ...
              async function comparePassword(password, hash) {
                 ...
                 console.log(result);
                 if(user.email== req.body.email && (result) && user.approved==1)
                 {
                  flig=1;
                  console.log("same"+flig);
                  console.log("user is"+ user);
                   var loginuser=user;
                  console.log("same"+loginuser);
...
//admin accept or reject people
 ...
 //get  all post of corresponding subject and also check if is premiun user.omly premim user have do comment and rarting
 //and comment dispay 
 ...
 router.get('/go/:id/:subject',function(req,res)
 {
  var data;
   const id=req.params.id;
    const subject=req.params.subject;
    aftercommentsubmitsubject=subject;//this variable for afterr cmment submit page is refresh .so again get corresponding thata subject 
      
      if(req.session.user)
      {
        //take artcle of only correspnding subject
       ...
       Articleuser.find({$and:[{subject:id},{approved:1}]},function(err, response){
        ...
        comment.find(function (err,response2)
        {
          ...
          aftercommentsubmitarticle=data;//this variable for after comment submit again comment show for user
         
      // res.render('sportsarticleallpost.pug',{article:data});
      //pass corresponding articleid session details for check premium or basicuser 
 ...
 //comments submit: comment submit  indb rating submit in articledb and check total rating is >30.if >30 update it is premium user
          ...
          if(err)
            ...
            //update rating
                ...
                else
               {
                 console.log(2);
                 console.log(response);
                  //again take all  rating of that user of corresponding useremail of all articles
                 Articleuser.find({$and:[{email:email},{approved:1}]},function(err, response){
                  ...
                  console.log("ratingsum" + ratingsum)
                    ...
                    {  
                      var ratesum=0;
                      //sum of the all rating of that user
